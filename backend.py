...
# Local Whisper for transcription (replacing OpenAI API)
import whisper
...
try:
    ...
    logger.info("LangChain available for RAG framework")
except ImportError:
...
# Import AudioSegment for pydub fallback
try:
    from pydub import AudioSegment
except ImportError:
    AudioSegment = None
...
try:
    ...
    logger.info("Google Generative AI (Gemini) available")
except ImportError:
    GEMINI_AVAILABLE = False
    logger.warning("Google Generative AI not available. Install with: pip install google-generativeai")

# Speaker diarization imports (optional)
try:
...
VECTORSTORE_PATH = os.getenv('VECTORSTORE_PATH', './unified_vectorstore')
MEMORY_THRESHOLD = int(os.getenv('MEMORY_THRESHOLD', 80))
WHISPER_MODEL_SIZE = os.getenv('WHISPER_MODEL_SIZE', 'tiny')
WHISPER_DEVICE = os.getenv('WHISPER_DEVICE', 'cpu')
MAX_CONTENT_LENGTH = 500 * 1024 * 1024  # 500MB limit for local Whisper

# Initialize Flask app
app = Flask(__name__)
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
app.config['MAX_CONTENT_LENGTH'] = 500 * 1024 * 1024  # 500MB limit for local Whisper
CORS(app)
...
ALLOWED_EXTENSIONS = {'mp4', 'avi', 'mov', 'mkv', 'wmv', 'flv', 'webm', 'mp3', 'wav', 'm4a', 'aac', 'ogg'}
...
# Memory management for 4GB RAM optimization
class MemoryManager:
    """Memory management for 4GB RAM systems"""
    
    def __init__(self, threshold=80):
        self.threshold = threshold
        logger.info(f"Memory manager initialized with {threshold}% threshold")
...
# Gemini embeddings for vector operations
class GeminiEmbeddings:
    ...
    def __init__(self, api_key=None):
        ...
        if self.api_key:
            ...
            logger.warning("No Gemini API key provided, embeddings disabled")
            self.model = None
...
# Unified vector store using ChromaDB
class UnifiedVectorStore:
    """Unified vector store using ChromaDB and Gemini embeddings"""
    
    def __init__(self, vectorstore_path="./unified_vectorstore", gemini_api_key=None):
        self.vectorstore_path = vectorstore_path
        self.gemini_embeddings = GeminiEmbeddings(gemini_api_key)
        ...
        try:
            ...
            logger.info(f"Unified vector store initialized at {vectorstore_path}")
    ...
    def save_vectorstore(self):
        """Save vector store (ChromaDB auto-saves)"""
        try:
            if self.client:
    ...
    def add_transcript(self, transcript: str, metadata: Dict[str, Any]) -> str:
        ...
        try:
            ...
            doc_id = f"doc_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{hash(transcript) % 10000}"
            ...
            logger.info(f"Added transcript to vector store: {doc_id}")
            return doc_id
            
        except Exception as e:
            logger.error(f"Error adding transcript to vector store: {e}")
            logger.warning("Vector store operation failed, but transcription completed successfully")
            # Don't raise the exception to avoid breaking the main transcription flow
            return None
    ...
    def add_documents(self, documents: List[Document]):
        ...
        try:
            ...
            ids = [f"doc_{i}_{datetime.now().strftime('%Y%m%d_%H%M%S')}" for i in range(len(documents))]
    ...
    def get_retriever(self):
        ...
        # For now, return a simple retriever
        return self
...
class UnifiedChatBot:
    ...
    def __init__(self, unified_vectorstore: UnifiedVectorStore, gemini_client=None):
        ...
        if self.gemini_api_key:
            ...
            logger.info("Unified chatbot initialized with Gemini")
        else:
            logger.warning("No Gemini API key provided, chatbot disabled")
            self.model = None
    ...
    def chat(self, user_question: str, n_context_results: int = 5) -> str:
        ...
        try:
            if not self.model:
                return "Chatbot not available. Please check Gemini API key."
            
            # Search for relevant context
            similar_docs = self.unified_vectorstore.similarity_search(user_question, n_context_results)
...
# MeetingBaas client for API integration
...
# Google Drive uploader for text files
...
# Video transcriber with customer support call analysis capabilities
class VideoTranscriber:
    """Enhanced video transcriber with customer support call analysis capabilities"""
    
    def __init__(self, gemini_api_key=None, google_drive_uploader=None, unified_vectorstore=None):
        # Initialize local Whisper for transcription
        try:
            self.whisper_model = whisper.load_model(WHISPER_MODEL_SIZE)
            logger.info(f"Local Whisper model '{WHISPER_MODEL_SIZE}' loaded successfully")
        except Exception as e:
        ...
        try:
            ...
            if hf_token:
                ...
                logger.warning("Speaker diarization not available (requires HF_TOKEN)")
        except Exception as e:
    ...
    def transcribe_audio(self, audio_path, language=None, prompt=None):
        try:
            ...
            # Check file size (local Whisper has no practical limit, but we'll warn for very large files)
            file_size = os.path.getsize(audio_path)
            max_size = 500 * 1024 * 1024  # 500MB limit for local Whisper
            
            if file_size > max_size:
                logger.warning(f"Audio file is very large ({file_size / (1024*1024):.1f}MB). Processing may take time.")
        ...
        try:
            ...
            if add_to_vectorstore and self.unified_vectorstore:
                try:
                    ...
                    result["vectorstore_error"] = "Vector store operation failed, but transcription completed successfully"
...
def initialize_services():
    global transcriber, unified_vectorstore, meeting_baas_client, memory_manager
    ...
    try:
        memory_manager = MemoryManager(MEMORY_THRESHOLD)
        logger.info(f"Memory manager initialized with {MEMORY_THRESHOLD}% threshold")
    except Exception as e:
    ...
    try:
        if not GEMINI_API_KEY:
            logger.warning("GEMINI_API_KEY not provided, vector store will be disabled")
            unified_vectorstore = None
        else:
    ...
    # Initialize transcriber with local Whisper
    try:
        transcriber = VideoTranscriber(GEMINI_API_KEY, drive_uploader, unified_vectorstore)
        logger.info("Video transcriber initialized successfully with local Whisper")
    except Exception as e:
...
def upload_video():
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            if not transcriber:
                return jsonify({"error": "Transcriber service not available. Please check server logs and ensure local Whisper is properly installed."}), 500
            ...
            try:
                ...
                # Add enhanced analysis if transcription was successful
                if result and result.get("transcript"):
                    try:
                        # Extract insights
                        if insights_extractor:
                            insights_result = insights_extractor.extract_all_insights(result["transcript"])
                            result["insights"] = insights_result
                            logger.info("Insights extracted successfully")
                        
                        # Generate summary
                        if conversation_summarizer:
                            summary_result = conversation_summarizer.generate_summary(result["transcript"])
                            result["summary"] = summary_result
                            logger.info("Summary generated successfully")
                        
                        # Evaluate call quality
                        if call_evaluator:
                            evaluation_result = call_evaluator.evaluate_call_quality(result["transcript"])
                            result["evaluation"] = evaluation_result
                            logger.info("Call evaluation completed successfully")
...
def comprehensive_analysis():
    ...
    try:
        ...
        analysis_results = transcriber.perform_comprehensive_analysis(
...
def extract_insights():
    ...
    try:
        if not transcriber or not transcriber.insights_extractor:
        ...
        # Extract insights
        insights = transcriber.insights_extractor.extract_all_insights(transcript)
...
def generate_summary():
    ...
    try:
        if not transcriber or not transcriber.conversation_summarizer:
        ...
        # Generate summary
        summary = transcriber.conversation_summarizer.generate_summary(transcript, insights)
        brief_summary = transcriber.conversation_summarizer.generate_brief_summary(transcript)
        key_phrases = transcriber.conversation_summarizer.extract_key_phrases(transcript)
...
def evaluate_call():
    ...
    try:
        if not transcriber or not transcriber.call_evaluator:
        ...
        # Evaluate call quality
        evaluation = transcriber.call_evaluator.evaluate_call_quality(transcript, insights, summary)
        performance_metrics = transcriber.call_evaluator.calculate_performance_metrics(transcript)
...
def agent_analysis():
    ...
    try:
        if not transcriber or not transcriber.support_agent:
        ...
        agent_analysis = transcriber.support_agent.analyze_call_for_actions(transcript, insights, summary)
        similar_cases = transcriber.support_agent.suggest_similar_cases(transcript)
...
def enhanced_chat():
    ...
    try:
        ...
        enhanced_response = transcriber.support_agent.create_enhanced_rag_response(query, transcript)
...
def perform_diarization():
    ...
    try:
        if not transcriber or not transcriber.speaker_diarizer:
        ...
        # Perform diarization
        diarization_result = transcriber.speaker_diarizer.perform_diarization(file_path)
...
if __name__ == '__main__':
    ...
    print("📊 Core Services:")
    print("   ✓ Basic transcription service (using Local Whisper)")
    if meeting_baas_client:
