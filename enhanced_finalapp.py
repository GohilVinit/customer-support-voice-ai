...
if st.session_state.backend_running:
    st.sidebar.header("Meeting Recording")
...
if st.session_state.backend_running:
    st.sidebar.header("🎯 Enhanced Call Analysis")
    ...
    if analysis_type == "Comprehensive Analysis":
        st.sidebar.subheader("📊 Comprehensive Call Analysis")
        
        # File upload for comprehensive analysis
        analysis_file = st.sidebar.file_uploader(
        ...
        if analysis_file:
            ...
            enable_diarization = st.sidebar.checkbox("Enable Speaker Diarization", value=True)
            enable_insights = st.sidebar.checkbox("Enable Insights Extraction", value=True)
            enable_summary = st.sidebar.checkbox("Enable Call Summarization", value=True)
            enable_evaluation = st.sidebar.checkbox("Enable Quality Evaluation", value=True)
            enable_agent = st.sidebar.checkbox("Enable Agent Analysis", value=True)
            
            language = st.sidebar.selectbox("Language", ["en", "es", "fr", "de", "it", "pt", "ru", "ja", "ko", "zh"])
            
            if st.sidebar.button("🚀 Start Comprehensive Analysis"):
                # Save file temporarily
                ...
                with st.spinner("Performing comprehensive analysis... This may take several minutes."):
                    ...
                    if result:
                        ...
                        st.sidebar.success("✅ Comprehensive analysis completed!")
                    else:
        ...
        st.sidebar.subheader("🔍 Individual Analysis Components")
                ...
                if st.button("📊 Extract Insights"):
                    with st.spinner("Extracting insights..."):
                        ...
                        if insights_result:
                            ...
                            st.sidebar.success("✅ Insights extracted!")
                ...
                if st.button("⭐ Evaluate Quality"):
                    with st.spinner("Evaluating call quality..."):
                        ...
                        if evaluation_result:
                            ...
                            st.sidebar.success("✅ Quality evaluated!")
                ...
                if st.button("🤖 Agent Analysis"):
                    with st.spinner("Performing agent analysis..."):
                        ...
                        if agent_result:
                            ...
                            st.sidebar.success("✅ Agent analysis completed!")
    
    elif analysis_type == "Speaker Diarization":
        st.sidebar.subheader("🎤 Speaker Diarization")
        
        diarization_file = st.sidebar.file_uploader(
        ...
        if diarization_file:
            if st.sidebar.button("🎤 Perform Diarization"):
                # Save file temporarily
                ...
                with st.spinner("Performing speaker diarization..."):
                    ...
                    if result:
                        ...
                        st.sidebar.success("✅ Diarization completed!")
                    else:
                        st.sidebar.error("❌ Diarization failed")
        ...
        st.sidebar.subheader("💬 Enhanced Chat")
        
        # Optional transcript for context
        chat_transcript = st.sidebar.text_area(
            "Optional: Enter transcript for context",
            height=100,
            placeholder="Paste transcript for enhanced chat context..."
        )
        
        chat_query = st.sidebar.text_input("Enter your question:")
        
        if chat_query:
            if st.sidebar.button("💬 Enhanced Chat"):
                with st.spinner("Processing enhanced chat..."):
                    result = enhanced_chat_backend(chat_query, chat_transcript if chat_transcript else None)
                    
                    if result:
                        st.session_state.enhanced_chat_result = result
                        st.sidebar.success("✅ Enhanced chat completed!")
                    else:
                        st.sidebar.error("❌ Enhanced chat failed")

# LLM Initialization (removed NVIDIA dependency)
# try:
...
# Using OpenAI as fallback LLM
llm = None  # Will use OpenAI fallback
...
if user_input:
    if not st.session_state.backend_running:
        st.warning("Backend server must be running for unified chat. Using OpenAI fallback...")
        fallback_answer = openai_fallback(user_input)
        st.subheader("Answer")
        st.write(fallback_answer)
        st.session_state.chat_history.append((user_input, fallback_answer, "OpenAI Fallback"))
    else:
        # Use unified backend chat
        try:
            chat_result = chat_with_backend(user_input)
            if chat_result:
                st.subheader("Answer from Unified Data")
                answer = chat_result.get("response", "")
                st.write(answer)
                st.session_state.chat_history.append((user_input, answer, "Unified Data"))
            else:
                # Fallback to OpenAI
                st.subheader("Answer")
                st.write("No relevant data found in unified embeddings. Using OpenAI fallback...")
                fallback_answer = openai_fallback(user_input)
                st.write(fallback_answer)
                st.session_state.chat_history.append((user_input, fallback_answer, "OpenAI Fallback"))
        except Exception as e:
            st.error(f"Error querying unified data: {e}")
            # Fallback to OpenAI
            fallback_answer = openai_fallback(user_input)
            st.subheader("Answer")
            st.write(fallback_answer)
...
if st.session_state.backend_running:
    st.subheader("Search Unified Data")
    search_query = st.text_input("Search documents and transcripts:")
    
    if search_query:
        search_results = search_unified_data_backend(search_query)
        if search_results and search_results.get('results'):
            ...
            for i, result in enumerate(search_results['results']):
                ...
                with st.expander(f"Result {i+1} from {doc_type}: {filename}"):
        ...
        with tab7:
            ...
            if performance:
                ...
                with col1:
                    ...
                    st.metric("Word Count", performance.get('word_count', 0))
                    st.metric("Sentence Count", performance.get('sentence_count', 0))
                    st.metric("Avg Words/Sentence", f"{performance.get('avg_words_per_sentence', 0):.1f}")
                
                with col2:
                    st.write("**Performance Metrics:**")
                    st.metric("Call Duration", f"{performance.get('call_duration_seconds', 0):.1f}s")
                    st.metric("Words per Minute", f"{performance.get('words_per_minute', 0):.1f}")
                    st.metric("Efficiency Score", f"{performance.get('efficiency_score', 0):.1f}")
            else:
